# [Input] 项目根目录 `src` 包与 Minimax 运行时配置（API Key/模型）。
# [Output] 提供可执行的 Minimax 全流程使用示例入口。
# [Pos] examples 层 Minimax 示例脚本，覆盖 chat/stream/agent/tool/context 流程。
...
try:
    ...
    if exc.name != "src":
        raise
    # 兼容 `python examples/minimax_usage.py` 直接运行场景
...
async def basic_chat_example():
    ...
    ai = create_ai(provider=PROVIDER, model=MODEL)
...
async def provider_switching_example():
    ...
    for provider_name in providers:
        try:
            ...
            print(f"供应商 {provider_name}: {error}")
        print()
